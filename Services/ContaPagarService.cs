...
namespace ContaMente.Services
{
    public class ContaPagarService : IContaPagarService
    {
        ...
        public async Task<List<ContaPagarDto>> GetContasPagar(
            int[]? categoriasIds,
            int[]? responsaveisIds,
            ...
            string userId)
        {
            var query = _contaPagarRepository.GetContasPagar(userId);
        ...
        public async Task<ContaPagarDto?> GetContaPagarById(int id, string userId)
        {
            var conta = await _contaPagarRepository.GetContaPagarById(id, userId);
            return conta == null ? null : MapToDto(conta);
        }

        public async Task<List<ContaPagarDto>> LancarContasPagar(CreateContaPagarDto dto, string userId)
        {
            if (dto.ValorTotal <= 0)
            ...
            var rateiosParcela = await CategoriaRateioHelper.CalcularRateio(
                ...
                valorParcela,
                userId,
                entradaEsperada: false);
            var grupoLancamentoId = Guid.NewGuid();
            var resultado = new List<ContaPagarDto>();

            for (int i = 0; i < dto.NumeroParcelas; i++)
            {
                var vencimentoOriginal = CalcularVencimento(dto.PrimeiroVencimento, dto.TipoIntervalo, dto.Intervalo, i);
                var vencimentoAjustado = await AjustarVencimentoAsync(vencimentoOriginal);
                ...
                var conta = new ContaPagar
                {
                    ...
                    Vencimento = DateTime.SpecifyKind(vencimentoAjustado, DateTimeKind.Utc),
                    Status = StatusDuplicataEnum.Aberta,
                    CategoriasRateio = rateiosParcela.Select(r => new ContaPagarCategoria
                    {
                ...
                var criada = await _contaPagarRepository.CreateContaPagar(conta);
        ...
        public async Task<ContaPagarDto?> UpdateContaPagar(int id, UpdateContaPagarDto dto, string userId)
        {
            var conta = await _contaPagarRepository.GetContaPagarById(id, userId);
            if (conta == null)
                return null;
            ...
            if (categoriasAlteradas)
            {
                ...
                _context.ContasPagarCategorias.RemoveRange(conta.CategoriasRateio);
                conta.CategoriasRateio = rateios.Select(r => new ContaPagarCategoria
                {
            ...
            var atualizada = await _contaPagarRepository.UpdateContaPagar(conta);
            return atualizada == null ? null : MapToDto(atualizada);
        }

        public async Task<ContaPagarDto?> BaixarContaPagar(int id, BaixaContaPagarDto dto, string userId)
        {
            var conta = await _contaPagarRepository.GetContaPagarById(id, userId);
            if (conta == null)
                return null;
            ...
            _context.ContasPagarMovimentacoes.Add(new ContaPagarMovimentacao
            {
                ContaPagarId = conta.Id,
        ...
        public async Task<ContaPagarDto?> MarcarComoPaga(int id, string userId)
        {
            var conta = await _contaPagarRepository.GetContaPagarById(id, userId);
            if (conta == null)
                return null;

            RecalcularStatus(conta, conta.ValorParcela);
            var atualizada = await _contaPagarRepository.UpdateContaPagar(conta);
            return atualizada == null ? null : MapToDto(atualizada);
        }

        public async Task<bool> DeleteContaPagar(int id, string userId)
        {
            var conta = await _contaPagarRepository.GetContaPagarById(id, userId);
            if (conta == null)
                return false;
            ...
            return await _contaPagarRepository.DeleteContaPagar(conta);
        }

        private static void RecalcularStatus(ContaPagar conta, decimal valorBaixado)
        {
        ...
        private async Task<DateTime> AjustarVencimentoAsync(DateTime data)
        {
            var ajustada = data;

            while (IsFimDeSemana(ajustada) ||
                   await _feriadoService.IsFeriadoNacionalAsync(ajustada))
            {
                ajustada = ajustada.AddDays(1);
            }

            return ajustada;
        }

        private static bool IsFimDeSemana(DateTime data)
            => data.DayOfWeek == DayOfWeek.Saturday || data.DayOfWeek == DayOfWeek.Sunday;

        private static ContaPagarDto MapToDto(ContaPagar c)
        {
        ...
        private static List<MovimentacaoCategoria> CriarRateioMovimentacaoDaBaixa(
            List<ContaPagarCategoria> rateiosDuplicata,
            decimal valorBaixa)
        {
        ...
        private static void RecalcularRateioExistente(List<ContaPagarCategoria> rateios, decimal novoTotal)
        {
            decimal acumulado = 0;
